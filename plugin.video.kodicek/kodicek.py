...
def parse_size_to_bytes(size_str: str) -> int:
    """Converts a size string like '11GB' or '444MB' to bytes."""
...
def get_tmdb_api_key():
    ...
    if not tmdb_key:
        # Fallback to default key and save it
...
def router(paramstring):
    ...
    if action == "search":
        ...
        if search_term:
            ...
            if not files:
                ...
                for file_item in files:
                    ...
                    xbmcplugin.addDirectoryItem(handle=addon_handle, url=url, listitem=li, isFolder=False)
        xbmcplugin.endOfDirectory(addon_handle, succeeded=True)
    elif action == "search_sdilej_test":
        xbmc.log("Kodíček: search_sdilej_test – dialog start", xbmc.LOGINFO)
        query = xbmcgui.Dialog().input(
            f"{plugin_name} – Test Sdílej.cz", type=xbmcgui.INPUT_ALPHANUM
        )
        if not query:
            xbmcplugin.endOfDirectory(addon_handle, succeeded=False)
            return

        results = sdilej.search(query, limit=100)

        if not results:
            xbmcgui.Dialog().notification(plugin_name,
                                          "Sdílej.cz – nic nenalezeno.",
                                          xbmcgui.NOTIFICATION_INFO)
            xbmcplugin.endOfDirectory(addon_handle, succeeded=True)
            return

        xbmcplugin.setPluginCategory(addon_handle, f"Sdílej.cz: {query}")
        xbmcplugin.setContent(addon_handle, 'videos')

        for r in results:
            li = xbmcgui.ListItem(label=r["title"])
            size_in_bytes = parse_size_to_bytes(r.get("size"))
            li.setInfo("video", {"title":  r["title"],
                                 "size":   size_in_bytes})
            li.setProperty("IsPlayable", "true")
            play_url = (f"{BASE_URL_PLUGIN}?action=play_sdilej"
                        f"&url={urllib.parse.quote(r['url'])}"
                        f"&title={urllib.parse.quote(r['title'])}")
            xbmcplugin.addDirectoryItem(addon_handle, play_url, li, isFolder=False)

        xbmcplugin.endOfDirectory(addon_handle, succeeded=True)
    elif action == "play_sdilej":
        page_url = params.get("url")
        title    = urllib.parse.unquote_plus(params.get("title", "Sdílej.cz"))
        ...
        stream = sdilej.get_download_url(page_url)
        if not stream:
            xbmcgui.Dialog().notification(plugin_name,
                                          "Nepodařilo se získat download-link.",
                                          xbmcgui.NOTIFICATION_ERROR)
            xbmcplugin.setResolvedUrl(addon_handle, False, xbmcgui.ListItem())
            return
...
def display_main_menu():
    ...
    li_sdilej_test = xbmcgui.ListItem(label="Vyhledávání – test Sdílej.cz")
    li_sdilej_test.setArt({'icon': 'DefaultAddonsSearch.png'})
    xbmcplugin.addDirectoryItem(
        handle=addon_handle,
        url=f"{BASE_URL_PLUGIN}?action=search_sdilej_test",
        listitem=li_sdilej_test,
        isFolder=True
    )
